...
const Users: React.FC = () => {
  ...
  const userElements: JSX.Element[] = users.map(user => {
    return (
      <tr className={`table-row`}
        ...
        <td>{user.lastName}</td>
        <td>{user.email}</td>
        <td><button className="btn btn-success" onClick={() => setUserAdmin(user)}>Definir admin</button> </td>
      </tr>);
  });
  ...
  const adminElements: JSX.Element[] = admins.map(admin => {
    return (
      <tr className={`table-row`}
        ...
        <td>{admin.email}</td>
        <td><button className="btn btn-danger" onClick={() => setUserNotAdmin(admin)}>Desfazer admin</button> </td>
      </tr>);
  });
  ...
  return (
    <Fragment>
      ...
      <p className="mb-4">Usuários aqui</p>

      <div className="row">
        <TopCard title="ADMINS" text={admins.length.toString()} icon="user-tie" class="primary" />
        <TopCard title="USUÁRIOS" text={users.length.toString()} icon="user" class="danger" />
      </div>
        ...
        <div className="col-xl-12 col-lg-12">
          <div className="card shadow mb-4">
            <div className="card-header py-3">
              <h6 className="m-0 font-weight-bold text-green">Lista de admins</h6>
      ...
      <div className="row">
        <div className="col-xl-12 col-lg-12">
          <div className="card shadow mb-4">
            <div className="card-header py-3">
              <h6 className="m-0 font-weight-bold text-green">Lista de usuários</h6>
              <div className="header-buttons">
              </div>
            </div>
            <div className="card-body">
